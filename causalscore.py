...
class Generator:
    def __init__(self, items=1000, nodes=4, timesteps=3, arcs_ratio=1.5, islands=1, values=2):
        self.items = items
        self.nodes = nodes
    ...
    def generate_structure(self):
        """
        Generate 2TBN structure from parameter settings.
        """
        all_names = []
    ...
    def generate_DBN(self, twodbn):
        """
        Generate DBN structure from 2TBN structure.
        """
        twodbn.generateCPTs()
    ...
    def generate_data(self,items=10000):
        """
        Generate data from ground-truth structure
        """
    ...
    def learn_causal_structure(self, subpoints):
        """
        Learn causal structure within a timestep.
        """
    ...
    def get_structure(self):
        """"
        Combine learned causal structure within a timestep and between timesteps.
        """
    ...
    def compare(self, subpoints):
        """
        Compare the ground-truth and learned structures based on the number of datapoints.

        Returns:
        dict: Parameters with corresponding scores.
        """
...
def test_method(num,arcs_ratio,nodes,timesteps, islands,datapoints,values, sema=None):
    """
    Run simulations given parameter settings and evaluations for learning causal structures.

    Parameters:
    num (int): Identifier for the simulation.
    arcs_ratio (float): Ratio of arcs in the generated structure.
    nodes (int): Number of nodes each timestep.
    timesteps (int): Number of timesteps.
    islands (int): Number of islands in the structure.
    datapoints (int): Number of max datapoints to generate.
    values (int): Cardinality of the nodes.
    sema (multiprocessing.Semaphore, optional): Semaphore for controlling concurrency.

    Returns:
    (Optional): Write results to files.
    int: The simulation identifier.
    """
...
def run_sims():
    """
    Run simulations in parallel batches using multiprocessing.

    This function runs simulations with various parameter combinations in parallel
    batches, taking advantage of multiple CPU cores. It utilizes the multiprocessing
    module for parallel processing and control over the number of concurrent jobs.
    """
    ...
    # Repetitions over all parameter settings
    batches = 30
    for batch in range(batches): 
        for i, params in reversed(list(enumerate(parameters))):  
            r, n, t, d, v = params
